...
;	walk.c:134: next:
00102$:
;	walk.c:135: printf("\033[2;1H% 8d% 8d% 8d", sp, cur.r, cur.c);
	push	ar2
	mov	ar7,@r0
	inc	r0
	mov	ar4,@r0
	...
	pop	ar2
	pop	ar6
;	walk.c:137: for (j = 0, f = 0; j < NMAX; j++) {
	mov	r3,#0x00
	push	ar0
	mov	a,_bp
	add	a,#0x12
...
00160$:
	...
	ljmp	00106$
00161$:
;	walk.c:138: if (!update(&t, &cur, j)) continue;
	push	ar2
	push	ar0
	mov	a,_bp
	add	a,#0x06
...
;	walk.c:144: j = rand() % NMAX;
	...
	pop	ar6
	mov	ar4,r5
;	walk.c:145: if (!update(&t, &cur, j)) continue;
	push	ar0
	mov	a,_bp
	add	a,#0x05
	mov	r0,a
...
00166$:
	...
	pop	ar2
	pop	ar6
;	walk.c:156: if (!stpop(&cur)) goto term;
	push	ar0
	mov	a,_bp
	add	a,#0x03
	mov	r0,a
...
;	walk.c:192: for (i = 0; i < (2 * REG); i++) {
	...
	mov	@r0,a
00115$:
;	walk.c:193: neigh[i].r = neigh_tmpl[i].r * (1 + rand() % 4);
	push	ar7
	mov	a,_bp
	add	a,#0x05
	mov	r0,a
...
00186$:
	...
	inc	dptr
	movx	@dptr,a
;	walk.c:194: neigh[i].c = neigh_tmpl[i].c * (1 + rand() % 4);
	mov	a,r3
	add	a,#_neigh
	mov	r6,a
	mov	a,r4
...
00187$:
	...
	inc	dptr
	movx	@dptr,a
;	walk.c:195: printf("% 8d% 8d", neigh[i].r, neigh[i].c);
	mov	dpl,r6
	mov	dph,r7
	movx	a,@dptr
	mov	r6,a
...
00188$:
	...
	ljmp	00115$
00189$:
;	walk.c:198: OE76 = OE76_0;
	mov	r0,#_OE76
	mov	@r0,#0x3f
;	walk.c:199: flashOE(OE76_NC);
	mov	dpl,#0x00
	push	ar7
	push	ar1
	lcall	_flashOE
...
;	assignBit
	clr	_EA
;	walk.c:212: puts("\033[2J\033[?25h");
	mov	dptr,#___str_7
	mov	b,#0x80
	lcall	_puts
;	walk.c:216: __endasm;
...
;------------------------------------------------------------
;	walk.c:221: static void stinit(void) {
;	-----------------------------------------
;	 function stinit
;	-----------------------------------------
_stinit:
...
;------------------------------------------------------------
;	walk.c:226: static int stpush(struct node *t) {
;	-----------------------------------------
;	 function stpush
;	-----------------------------------------
_stpush:
	...
	mov	r6,dph
	mov	r7,b
;	walk.c:227: if (sp == (SMAX - 1)) return 0;
	mov	dptr,#_sp
	movx	a,@dptr
	mov	r3,a
	inc	dptr
...
;t                         Allocated to registers r5 r6 r7 
;------------------------------------------------------------
;	walk.c:233: static int stpop(struct node *t) {
;	-----------------------------------------
;	 function stpop
;	-----------------------------------------
_stpop:
