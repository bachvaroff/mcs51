...
;------------------------------------------------------------
;	walk.c:136: static volatile __xdata int *R = (__xdata int *)0xfffe;
...
_walk:
	...
	add	a,#0x0d
	mov	sp,a
;	walk.c:108: if (!qadd(nstart)) bang();
	mov	r5,dpl
	mov	r6,dph
	mov	r7,b
	push	ar7
	...
	pop	ar7
00102$:
;	walk.c:109: g[nstart->r][nstart->c] = 0xaa;
	mov	dpl,r5
	mov	dph,r6
	mov	b,r7
	lcall	__gptrget
...
;	walk.c:111: process:
	...
	mov	r4,a
00103$:
;	walk.c:112: if (!qget(&cur)) goto term;
	push	ar5
	mov	ar2,r1
	mov	r3,#0x00
	mov	r5,#0x40
...
00160$:
	...
	pop	ar5
	pop	ar6
;	walk.c:116: goto process;
	ljmp	00103$
00109$:
;	walk.c:119: printf("\033[%d;%dH.", cur.r + 4, cur.c + 1);
	push	ar5
	mov	ar2,@r0
	inc	r0
	mov	ar3,@r0
...
;	walk.c:125: g[t.r][t.c] = 0xaa;
	...
	mov	a,#0xaa
	movx	@dptr,a
;	walk.c:126: printf("\033[%d;%dHo", t.r + 4, t.c + 1);
	mov	r0,ar4
	mov	ar5,@r0
	inc	r0
	mov	ar7,@r0
...
;	walk.c:148: qinit();
	lcall	_qinit
;	walk.c:150: puts("\033[2J\033[?25l");
	mov	dptr,#___str_4
	mov	b,#0x80
	lcall	_puts
;	walk.c:152: while (i0) {
...
00196$:
	...
	subb	a,#0x80
	jc	00127$
;	walk.c:157: initial.r = rand() % ROWS;
	mov	a,_bp
	add	a,#0x05
	mov	r0,a
	push	ar7
	...
	mov	@r0,ar6
	dec	r0
;	walk.c:158: initial.c = rand() % COLS;
	mov	a,#0x02
	add	a,r1
	mov	r0,a
	push	ar1
	...
	mov	@r0,ar6
	dec	r0
;	walk.c:160: puts("\033[2J\033[?25l");
	mov	dptr,#___str_4
	mov	b,#0x80
	push	ar0
	lcall	_puts
	pop	ar0
	pop	ar1
;	walk.c:161: printf("\033[1;1H% 8u% 8d% 8d", N, initial.r, initial.c);
	mov	ar5,@r0
	inc	r0
	mov	ar6,@r0
	dec	r0
...
;	walk.c:163: for (i = 0; i < REG; i++) {
	...
	mov	@r0,a
00116$:
;	walk.c:164: neigh[i].r = neigh_tmpl[i].r * (1 + rand() % 4);
	push	ar7
	mov	a,_bp
	add	a,#0x0b
	mov	r0,a
...
00198$:
	...
	inc	dptr
	movx	@dptr,a
;	walk.c:165: neigh[i].c = neigh_tmpl[i].c * (1 + rand() % 4);
	mov	a,r3
	add	a,#_neigh
	mov	r6,a
	mov	a,r4
...
00199$:
	...
	inc	dptr
	movx	@dptr,a
;	walk.c:166: printf("% 8d% 8d", neigh[i].r, neigh[i].c);
	mov	dpl,r6
	mov	dph,r7
	movx	a,@dptr
	mov	r6,a
...
00200$:
	...
	ljmp	00116$
00201$:
;	walk.c:169: for (i = REG; i < (2 * REG); i++) {
	mov	a,_bp
	add	a,#0x0b
	mov	r0,a
	mov	@r0,#0x04
...
;	assignBit
	clr	_EA
;	walk.c:186: puts("\033[2J\033[?25h");
	mov	dptr,#___str_7
	mov	b,#0x80
	lcall	_puts
;	walk.c:190: __endasm;
...
;------------------------------------------------------------
;	walk.c:195: static void qinit(void) {
;	-----------------------------------------
;	 function qinit
;	-----------------------------------------
_qinit:
...
;------------------------------------------------------------
;	walk.c:200: static int qadd(struct node *t) {
;	-----------------------------------------
;	 function qadd
;	-----------------------------------------
_qadd:
	...
	mov	r5,dph
	mov	r6,b
;	walk.c:201: if (((hp + 1) % QMAX) == tp) return 0;
	mov	dptr,#_hp
	movx	a,@dptr
	mov	r3,a
	inc	dptr
...
;------------------------------------------------------------
;	walk.c:207: static int qget(struct node *t) {
;	-----------------------------------------
;	 function qget
;	-----------------------------------------
_qget:
