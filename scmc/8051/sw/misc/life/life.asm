...
;	life.c:118: memset(pu, 0, sizeof (pu));
	...
	dec	sp
	dec	sp
;	life.c:190: printstr("\033[2J\033[mINIT\r\n");
	mov	r5,#___str_4
	mov	r6,#(___str_4 >> 8)
	mov	r7,#0x80
;	life.c:48: return;
...
;	life.c:163: n = -u[A2D(W, y, x)];
	...
	subb	a,r5
	movx	@dptr,a
;	life.c:164: for (y1 = y - 1; y1 <= y + 1; y1++)
	dec	r6
	cjne	r6,#0xff,00387$
	dec	r7
00387$:
...
00173$:
	...
	ljmp	00139$
00388$:
;	life.c:165: for (x1 = x - 1; x1 <= x + 1; x1++)
	mov	dptr,#_x
	movx	a,@dptr
	mov	r4,a
	inc	dptr
...
00390$:
	...
	ljmp	00174$
00391$:
;	life.c:166: n += u[A2D(W, (y1 + H) % H, (x1 + W) % W)];
	mov	dptr,#_y1
	movx	a,@dptr
	mov	r4,a
	inc	dptr
...
;	life.c:164: for (y1 = y - 1; y1 <= y + 1; y1++)
	...
	ljmp	00173$
00139$:
;	life.c:168: nu[A2D(W, y, x)] = (n == 3) || ((n == 2) && u[A2D(W, y, x)]);
	mov	a,r7
	swap	a
	rl	a
	anl	a,#0xe0
...
00198$:
	...
	mov	dph,r3
	movx	@dptr,a
;	life.c:170: if (pu[A2D(W, y, x)] != nu[A2D(W, y, x)]) cycle2 = 0;
	mov	dptr,#_y
	movx	a,@dptr
	mov	r6,a
	inc	dptr
...
00396$:
	...
	movx	@dptr,a
00141$:
;	life.c:171: if (u[A2D(W, y, x)] != nu[A2D(W, y, x)]) fixed = 0;
	mov	dptr,#_y
	movx	a,@dptr
	mov	r6,a
	inc	dptr
...
;	life.c:159: for (y = 0; y < H; y++) {
	...
	ljmp	00177$
00399$:
;	life.c:175: memcpy(pu, u, sizeof (u));
	clr	a
	push	acc
	mov	a,#0x04
	push	acc
	...
	add	a,#0xfb
	mov	sp,a
;	life.c:176: memcpy(u, nu, sizeof (nu));
	clr	a
	push	acc
	mov	a,#0x04
	push	acc
